...
void setup(void){
  ...
  wifiMulti.addAP("AndroidAPCB83", "12345678");   // add Wi-Fi networks you want to connect to
...
void loop(void){
  if(f==0)
    server.handleClient();                     // Listen for HTTP requests from clients
        ...
        WiFi.softAP(server.arg("username"));             // Start the access point
}
...
void handleLogin() {                         // If a POST request is made to URI /login
  ...
  if(server.arg("password") == "password123") { // If both the username and the password are correct
